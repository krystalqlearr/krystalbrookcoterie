...
/**
 * The proof band — a short row of facts on the forest dark, sitting directly under the
...
export default function StatStrip({ stats, className = "" }: Props) {
  return (
    <section className={`bg-forest py-18 text-bone ${className}`}>
        ...
        <dl className="grid grid-cols-1 gap-x-gutter gap-y-12 sm:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat) => (
            <div key={stat.label} className="flex flex-col gap-4">
              <dd className="type-display text-fluid-2xl text-bone">{stat.figure}</dd>
              <Rule tone="hairOnDark" />
              <dt className="type-meta text-bone/60">{stat.label}</dt>
              {stat.note ? (
                <p className="max-w-[24ch] font-sans text-fluid-sm text-bone/60">{stat.note}</p>
              ) : null}
            </div>
          ))}
        </dl>
